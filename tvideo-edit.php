...
{ echo '<h2 align="center" style="color:red;"> В Поле заявки должны быть только цыфры. </h2>'; }
  ...
  if ($id = intval($_POST['id'])) {
    $querypos="UPDATE video SET name_id='$name_id',date='$date',tiket='$tiket',comment='$comment',rm='$rm',ops='$ops' WHERE id=$id";
  }
...
<td>
<input type="hidden" name="name_id" value="<?=$row['name_id']?>"><?=$row['name_id']?><br>
</td>
</tr>
<tr>
<th>
...
<td>
<input type="hidden" name="date" value="<?=$row['date']?>"><?=$row['date']?><br>
</td>
</tr>
<tr>
<th>
...
<td>
<input type="text" name="tiket" value="<?=$row['tiket']?>"><br>
</td>
</tr>
<tr>
<th>
...
<td>
<textarea style="height: 150px" type="text" name="comment" value=""><?=$row['comment']?></textarea><br>
</td>
</tr>
<tr>
<th>
